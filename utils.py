...
def init_driver(chrome_path, chromedriver_path):
    ...
    chrome_options.binary_location = "C:/Program Files (x86)/Google/Chrome/chrome.exe"
    ...
    while True:
        ...
        if(not more1):
            try:
                ...
                ss=xd.find_elements_by_xpath("//div[@id='oc-background-section']/span[@class=background-details]/div/section/div/section[@id=experience-section]/ul/li/section/div/div/button")
        ...
        if(not more2):
            try:
                xd=self.driver.find_element_by_class_name("profile-detail")
                ss=xd.find_element_by_xpath("//div/div/section/div/button")
                ss.click()
                more2=True
            except:
                print('NO')
